...
- Use `axiosErrorMessage(error)` from `apps/web/src/lib/axios-error.ts` to extract a mutation's server-sent `{ error }` message (returns `undefined` if `error` isn't an Axios error or has no `{ error }` body) — don't reimplement the `axios.isAxiosError(...) ? ... : undefined` check inline per component. See `apps/web/src/components/ReplyForm.tsx` or `UserFormDialog.tsx` for the reference usage (`const serverError = axiosErrorMessage(mutation.error)`, rendered via `<FieldError>` — see below).
...
- Render any field-level (`errors.<field>?.message`) or server (`axiosErrorMessage(...)`) validation message with `<FieldError message={...} />` (`apps/web/src/components/ui/field-error.tsx`) — it renders nothing when `message` is falsy, so callers don't need a separate `{condition && <p>...}` guard. Don't hand-write `<p className="text-sm text-destructive">{message}</p>` inline; that duplication is exactly what `FieldError` replaces. See `apps/web/src/components/UserFormDialog.tsx` or `ReplyForm.tsx` for the reference usage.
- For a page-level load-failure banner (a query's `isError` flag, not a dynamic message string — e.g. "Couldn't load tickets" in `TicketsPage.tsx`), use `<ErrorMessage show={...}>...</ErrorMessage>` (`apps/web/src/components/ui/error-message.tsx`) instead of a hand-written `{condition && <p className="mt-4 text-sm text-destructive">...</p>}`. It takes the static copy as `children` rather than a `message` prop, which is what distinguishes it from `FieldError`. See `UsersPage.tsx` or `TicketsPage.tsx` for the reference usage.
